...
class LoanCalculatorService {
    ...
    def genericSaveLoan(SecUser userInstance, def amount, String userMobileNumber){
        ...
        if(loanInstance.save(failOnError:true,flush:true)) {
            if(paymentConfig.sendAutoLoan && !userInstance.accountLocked) {
                ...
                if(valid) {
                    JSONObject output = paymentConfig.disbursementSrc.equals("Vodacom") ? mpesaService.processLoan(loanInstance.amount, loanInstance.request_unique, loanInstance.request_unique, userMobileNumber) : tigoPesaService.processLoan((int)loanInstance.amount, loanInstance.request_unique, loanInstance.request_unique, userMobileNumber)
