<%@ page import="finance.CardServices; loans.CardPayments; loans.LoanRequest;" %>
<!DOCTYPE html>
<html>
<head>
            ...
            <div class="page-title">
                <h5>
                    ...
                    <i class="icon-arrow-left52 position-left"></i> <span
                        class="text-semibold">Issued Card list</span>
                </h5>
            </div>
                ...
                <g:each in="${cardServicesList}" status="i" var="cardServicesInstance">
                    ...
                    <td>
                        <g:link class="create" controller="user" action="show" id="${cardServicesInstance?.user_id?.id}"
                            ...
                            ${fieldValue(bean: cardServicesInstance, field: "user_id.first_name") + " " + fieldValue(bean: cardServicesInstance, field: "user_id.middle_name") + " " + fieldValue(bean: cardServicesInstance, field: "user_id.last_name")}</td>
                        </g:link>
                ...
                <g:each in="${cardPaymentList}" status="i" var="cardPaymentInstance">
                    ...
                    <td>
                        <g:link class="create" controller="user" action="show" id="${cardPaymentInstance?.user_id?.id}"
                            ...
                            ${fieldValue(bean: cardPaymentInstance, field: "user_id.first_name") + " " + fieldValue(bean: cardPaymentInstance, field: "user_id.middle_name") + " " + fieldValue(bean: cardPaymentInstance, field: "user_id.last_name")}</td>
                        </g:link>
...
<g:if test="${com.tacip.loans.LoanRequest.countByUser_id(userInstance)>0}">

    <div class="page-header-content">
        <div class="page-title">
            <h4>
                <i class="icon-list position-left"></i> <span
                    class="text-semibold">Loan received (${loans.LoanRequest.countByUser_id(userInstance)})</span>
            </h4>
        </div>
    </div>
    ...
    <table class="table datatable-basic">
        ...
        <tbody>
        <g:each in="${com.tacip.loans.LoanRequest.findAllByUser_id(userInstance)}" status="i" var="loanInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'} ">
                <td>
                    <% def offset = 0
                    if (params.offset) {
                    ...
                    ${i + 1 + offset}</td>
                <td>
                    <g:link class="create" controller="user" action="show" id="${loanInstance?.user_id?.id}"
                            resource="${this.loanInstance}">
                        ${fieldValue(bean: loanInstance, field: "user_id.first_name") + " " + fieldValue(bean: loanInstance, field: "user_id.middle_name") + " " + fieldValue(bean: loanInstance, field: "user_id.last_name")}
                    </g:link>
                </td>
